...
export const AdminPage = () => {
    ...
    useEffect(() => {
        const fetchClaims = async () => {
            try {
                const response = await fetch('https://server-one-drab.vercel.app/api/claims/allClaims', {
                    method: 'POST',
                    headers: {
    ...
    const handleApproveClaim = async (claimId, userId, policyId, amount) => {
        try {
            const response = await fetch(`https://server-one-drab.vercel.app/api/claims/updateClaim/${claimId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
